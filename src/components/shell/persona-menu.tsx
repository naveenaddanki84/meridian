  ...
  return (
    <div className="relative">
      <button
        ...
        aria-expanded={open}
      >
        <span className="flex h-7 w-7 items-center justify-center rounded-full bg-spruce text-[11px] font-bold text-white">
          {persona.initials}
        </span>
        <span className="hidden text-left leading-tight md:block">
          <span className="block text-[13px] font-semibold text-ink">{persona.name}</span>
          <span className="block text-[11px] text-ink-faint">{ROLE_LABEL[persona.role]}</span>
        </span>
        <ChevronDown className="h-3.5 w-3.5 text-ink-faint" />
      </button>

      <Popover open={open} onClose={() => setOpen(false)} className="w-72 p-2">
        <p className="px-2 pb-1 pt-1.5 text-[11px] font-bold uppercase tracking-wide text-ink-faint">
          Demo · view as
        </p>
        {PERSONAS.map((p) => (
          <button
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="block truncate text-[11px] text-ink-faint">{p.title}</span>
            </span>
